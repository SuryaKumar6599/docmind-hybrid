...
async def health() -> HealthResponse:
    return HealthResponse(status="ok", runtime="local-fastapi-ollama")
...
async def root() -> dict[str, object]:
    return {
        ...
        "endpoints": {
            ...
            "ask": "POST /ask JSON {question, match_count}",
        },
    }
...
async def index_document(
    ...
    try:
        ...
        document_id = store.create_document(filename, metadata)
...
async def ask_question(
    ...
    try:
        ...
        sources = store.match_documents(query_embedding, body.match_count)
        answer, citations = LocalRAG(ollama).answer(body.question, sources)
