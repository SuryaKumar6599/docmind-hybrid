...
export default function Home() {
  ...
  async function uploadDocument() {
    ...
    try {
      ...
      setStatus(
        `Indexed ${data.document_name || file.name} (${data.chunks || 0} chunks)`
      );
      setFile(null);
            ...
            {messages.length === 0 ? (
              <div className="flex h-full min-h-96 items-center justify-center text-center text-ink/55">
                <p className="max-w-md text-lg">
                  Ask a question after indexing documents from your local
                  backend.
                </p>
              </div>
            ) : (
...
function MessageBubble({ message }: { message: ChatMessage }) {
  ...
  return (
    ...
    >
      {!isUser && (
        <Bot className="mt-2 shrink-0 text-moss" size={22} />
      )}
      <div
        className={`max-w-3xl rounded-lg px-4 py-3 ${
          isUser ? "bg-signal text-white" : "bg-paper text-ink"
        }`}
      >
          ...
          <div className="mt-4 space-y-2">
            {message.citations.map((c) => (
              ...
              >
                ...
                {c.quote && (
                  <p className="mt-2 text-ink/75">{c.quote}</p>
                )}
              </details>
            ))}
          </div>
      ...
      {isUser && (
        <User className="mt-2 shrink-0 text-signal" size={22} />
      )}
    </article>
  );
}
